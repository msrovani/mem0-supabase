            ...
            for edge in edges:
                try:
                    ...
                    source_id = conn.execute(
                        text("select id from nodes where name = :name"), {"name": edge["source"]}
                    ).scalar()
                    target_id = conn.execute(
                        text("select id from nodes where name = :name"), {"name": edge["target"]}
                    ).scalar()
    ...
    def search(self, query: str) -> List[Dict[str, Any]]:
        ...
        with self.engine.connect() as conn:
            try:
                ...
                for row in rows:
                    results.append({"source": row.source, "relation": row.relation, "target": row.target})
            except Exception as e:
