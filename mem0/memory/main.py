...
class Memory(MemoryBase):
    ...
    def search(
        ...
        *,
        ...
        # Layer 12: Ego Context Injection
        if self.config.enable_ego:
            id_filters = {"user_id": (filters or {}).get("user_id", "*"), "memory_type": "identity"}
...
class AsyncMemory(MemoryBase):
    ...
    async def _add_to_graph(self, messages, filters):
        """
        Add messages to the graph store asynchronously.
        """
        if not self.enable_graph or not self.graph:
            return None
        ...
        return await asyncio.to_thread(_sync_add_graph)
    ...
    async def search(
        ...
        *,
        ...
        # Layer 12: Ego Context Injection
        if self.config.enable_ego:
            # For async, we use the same vector_store.list (which is usually fast/sync-like metadata fetch)
            id_filters = {"user_id": (filters or {}).get("user_id", "*"), "memory_type": "identity"}
            identity_query = self.vector_store.list(filters=id_filters, limit=1)
            if identity_query and len(identity_query) > 0:
                results["persona_identity"] = identity_query[0].payload.get("data", "")
    ...
    async def recollect(
        ...
        """
        Human-like memory recollection asynchronously.
        Performs weighted ranking (Similarity + Importance + Recency) and associative jumps.
        """
        capture_event("mem0.recollect", self, {"query": query, "limit": limit, "sync_type": "async"})
