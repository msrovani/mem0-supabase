...
class AsyncMemory(MemoryBase):
    ...
    async def synthesize_identity(self, user_id: str, agent_id: Optional[str] = None) -> str:
        ...
        await self.add(f"CURRENT IDENTITY: {persona}", user_id=user_id, agent_id=agent_id, metadata={"memory_type": "identity"})
    ...
    async def _add_to_vector_store(
        ...
        async def process_fact_for_search(new_mem_content):
            ...
            surprise = self.surprise_engine.evaluate(embeddings, [
                {"id": m.id, "score": m.score} for m in existing_mems
            ])
            surprise_results[new_mem_content] = surprise
                ...
                try:
                    ...
                    if event_type == "ADD":
                        ...
                        if not surprise["is_surprising"] and surprise["best_match_id"]:
                            # Low surprise -> Reinforce existing memory asynchronously
                            async def reinforce_async(mem_id):
                                if self.lifecycle:
                                    await asyncio.to_thread(self.lifecycle.reinforce_memory, mem_id)
                                elif self.nexus:
                                    await asyncio.to_thread(self.nexus.reinforce_memory, mem_id)
                                return mem_id

                            task = asyncio.create_task(reinforce_async(surprise["best_match_id"]))
                            memory_tasks.append((task, resp, "REINFORCE", surprise["best_match_id"]))
                            continue
                        ...
                        if mem_id:
                            ...
                            memory_tasks.append((task, resp, "UPDATE", mem_id))
                    elif event_type == "DELETE":
                        mem_id = temp_uuid_mapping.get(resp.get("id"))
                        if mem_id:
                            task = asyncio.create_task(self._delete_memory(memory_id=mem_id))
                            memory_tasks.append((task, resp, "DELETE", mem_id))
    ...
    async def update(self, memory_id, data):
        ...
        existing_embeddings = await asyncio.to_thread(
             lambda: {data: self.embedding_model.embed(data, "update")}
        )
    ...
    async def delete_all(self, user_id: Optional[str] = None, agent_id: Optional[str] = None, run_id: Optional[str] = None):
        """
        ...
        memories = await asyncio.to_thread(self.vector_store.list, filters=filters, limit=10000)
        # unwrap if needed
        if isinstance(memories, (tuple, list)) and len(memories) > 0 and isinstance(memories[0], (list, tuple)):
             memories = memories[0]
        ...
        if self.enable_graph:
            if hasattr(self.graph.delete_all, "__await__"):
    ...
    async def _create_procedural_memory(self, messages, metadata=None, prompt=None, llm=None):
        ...
        try:
            if llm:
                procedural_memory = await llm.agenerate_response(messages=parsed_messages)
            else:
    ...
    async def _update_memory(self, memory_id, data, existing_embeddings, metadata=None):
        ...
        # Preserve session identifiers and enterprise metadata
    ...
    async def reset(self):
        ...
        if hasattr(self.db, "connection") and self.db.connection:
            await asyncio.to_thread(self.db.connection.execute, "DROP TABLE IF EXISTS history")
        ...
        if hasattr(self.vector_store, "reset"):
            ...
            # Recreate logic mostly sync for factories
            self.vector_store = VectorStoreFactory.create(
                self.config.vector_store.provider, self.config.vector_store.config
            )
    ...
    async def recollect(
        ...
        """
        Human-like memory recollection (Async).
        ...
        return await self.recollection.recollect_async(
            query=query, 
            filters=filters, 
            limit=limit, 
