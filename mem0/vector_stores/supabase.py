...
class Supabase(VectorStoreBase):
    ...
    def _preprocess_filters(self, filters: Optional[dict] = None) -> Optional[dict]:
        ...
        if filters is None:
            return None
        if any(isinstance(key, str) and key.startswith("$") for key in filters.keys()):
            return filters
        for value in filters.values():
            if isinstance(value, dict) and any(isinstance(k, str) and k.startswith("$") for k in value.keys()):
                return filters
        operator_map = {
            "eq": "$eq",
            "ne": "$ne",
        ...
        for key, value in filters.items():
            if value == "*":
                return filters
            if isinstance(value, dict) and any(op in operator_map for op in value.keys()):
                has_operator = True
                processed_filters[key] = {operator_map.get(op, op): v for op, v in value.items()}
            else:
    ...
    def create_col(self, embedding_model_dims: Optional[int] = None) -> None:
        ...
        try:
            # Check for HALFVEC support (requires pgvector 0.7.0+)
            if self.use_halfvec:
                # vecs might not support halfvec directly in get_or_create_collection
                # We can try to create the table manually via SQL if it doesn't exist
    ...
    def _hybrid_search(
        self, query_text: str, query_vector: List[float], limit: int, filters: Optional[dict]
    ) -> List[OutputData]:
        """
