...
GRAPH_MEMORY_EXTRACTION_PROMPT = """
You are an AI assistant capable of extracting entities and relationships from text for knowledge graph construction.
Your task is to identify key entities and the relationships between them in the given text.

Guidelines:
- Extract nodes (entities) and edges (relationships).
- Nodes should have a 'name' and optionally a 'label' (e.g., Person, Location, Organization).
- Edges should have a 'source', 'target', and 'relation' (e.g., knows, visited, works_at).
- Return the output in JSON format with 'nodes' and 'edges' keys.
...
Example Output:
{
  "nodes": [
    ...
    {"name": "Google", "label": "Organization"},
    {"name": "San Francisco", "label": "Location"}
  ],
  "edges": [
    {"source": "Alice", "target": "Google", "relation": "works_at"},
    {"source": "Alice", "target": "San Francisco", "relation": "lives_in"}
  ]
}
...
MEMORY_MERGE_PROMPT = """
You are a memory consolidation engine. Your goal is to merge a NEW FACT with an EXISTING MEMORY into a single, high-density, and consistent entry.

Guidelines:
1. Preserve all specific details (names, dates, quantities).
2. Resolve any contradictions by prioritizing the NEW FACT if it seems more recent or definitive.
3. If they convey the same thing, keep the one with more detail.
