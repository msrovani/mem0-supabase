...
def setup_mocks(mem_instance):
    ...
    # Mock do LLM
    mem_instance.llm.generate_response = MagicMock(
        return_value='{"facts": ["Usuário gosta de tecnologia"], "memory": [{"id": "0", "text": "Usuário gosta de tecnologia", "event": "ADD"}]}'
    )
...
async def run_extreme_test():
    ...
    config.vector_store.config = SupabaseConfig(
        connection_string="postgresql://mock:mock@localhost:5432/mock", collection_name="extreme_test"
    )
    ...
    mem_async.llm.generate_response = MagicMock(
        return_value='{"facts": ["Fato"], "memory": [{"id": "1", "text": "Fato", "event": "ADD"}]}'
    )
    ...
    mem_async.orchestrator.orchestrate_async = AsyncMock(
        return_value={"active_context": [], "background_context": "Resumo"}
    )
