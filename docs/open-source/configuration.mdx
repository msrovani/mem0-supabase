...
Use the flags below to enable the new high-agency layers:
...
config = MemoryConfig(
    ...
    dreaming_interval=10,
    enable_compaction=True,
    enable_heartbeat=True,
    heartbeat_interval=3600,
    enable_tool_state=True,
    enable_bridge=True
)
...
<AccordionGroup>
  ...
  <Accordion title="Hybrid Search Tuning">
    Ajuste `full_text_weight`, `semantic_weight` e `rrf_k` para calibrar a fusão RRF. Para ganho de qualidade no rerank, aumente `rerank_candidate_multiplier` para ampliar o pool de candidatos.
  </Accordion>
</AccordionGroup>
