...
<AccordionGroup>
  <Accordion title="Supabase (Graph-on-Postgres) ⭐ Recommended">
    Use Supabase's PostgreSQL with custom `nodes` and `edges` tables. No external graph database required!
...
**Setup**: Run `python scripts/setup_wizard.py` to create the required `nodes`, `edges` tables and `match_related_nodes` RPC function.

See [Supabase Innovations Architecture](/architecture/supabase_innovations) for details.
  </Accordion>
